...
def main():
    ...
    for sim_id, source, rates in iter_rate_matrices(args.npz, meta):
        ...
        np.divide(beta, alpha[:, None], out=omega, where=(alpha[:, None] != 0))

        row = {
            "simulation_id": sim_id,
            "source": source,
            "n_sites": rates.shape[0],
            "n_parameters": rates.shape[1],
            "alpha_mean": fmt_float(mean_finite(alpha)),
        }

        for j in range(beta.shape[1]):
            row[f"beta_class{j}_mean"] = fmt_float(mean_finite(beta[:, j]))
            row[f"omega_class{j}_mean"] = fmt_float(mean_finite(omega[:, j]))
            row[f"prop_pos_class{j}"] = fmt_float(float(np.mean(omega[:, j] > 1.0)))
            row[f"n_pos_class{j}"] = int(np.sum(omega[:, j] > 1.0))
