...
function reshape_probability_vector(model::SKBDIModel, probability_vector::Vector{Float64})
    return permutedims(reshape(probability_vector, reverse(model.grid_sizes)...),
        reverse(1:length(model.grid_sizes)))
end

function unreshape_probability_vector(model::SKBDIModel, probability_array::Array{Float64})
    return vec(permutedims(probability_array, reverse(1:length(model.grid_sizes))))
end
...
# TODO: Leverage the grid structure and use gaussian blur
...
function skbdifFUBAR(seqnames, seqs, treestring, tags, outpath, sampler::String; n_chains=1,
    ...
    total_time = @elapsed begin
        ...
        probability_array = reshape_probability_vector(model, probability_vector)
        new_probability_vector = unreshape_probability_vector(model, probability_array)
...
function main()
    ...
    tags = ["{G1}", "{G2}"]
    df, results = skbdifFUBAR(seqnames, seqs, treestring, tags, analysis_name, "nuts"; iters=1, n_chains=8)
