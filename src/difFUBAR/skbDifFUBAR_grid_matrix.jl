using Statistics, Combinatorics, Distributions, MCMCChains, LinearAlgebra, Phylo, FASTX, ImageFiltering, MolecularEvolution, CodonMolecularEvolution, Plots, EllipticalSliceSampling, AbstractMCMC, NNlib, StatsBase, ProgressMeter, JLD2, AdvancedHMC, LogDensityProblems, ForwardDiff, LogDensityProblemsAD, Zygote
import DSP # To avoid identifier conflicts DSP.conv and some other fct named conv
...
- grid_sizes: The number of grid points for each parameter.
...
function reshape_probability_vector(grid_sizes::Tuple, probability_vector::Vector{Float64})
    return permutedims(reshape(probability_vector, reverse(grid_sizes)...),
        reverse(1:length(grid_sizes)))
end

function unreshape_probability_vector(grid_sizes::Tuple, probability_array::Array{Float64})
...
function to_probability_vector(model::SKBDIModel, ambient_sample::AbstractVector{<:Real})
    ...
    for i in 1:size(model.masks, 1)
        ...
        suppression_factor = minimum(transition_function_values[model.mask_subset_indicators[i, :]])
...
function to_probability_vector(model::SKBDIModel, ambient_sample::AbstractVector{<:Real})
    ...
    for i in 1:size(model.masks, 1)
        ...
        suppression_factor = minimum(transition_function_values[model.mask_subset_indicators[i, :]]) # Change to min instead of mean here...
        suppression_weights = suppression_weights .* ifelse.(model.masks[i], suppression_factor, 1.0)
    end
    suppressed_probabilities = probability_vector .* suppression_weights
    if all(suppressed_probabilities .== 0.0)
...
function toves_ambient_to_parameter_transform(ambient_sample::AbstractVector{<:Real}, grid_sizes::Tuple, kernel_dim::Int64, suppression_dim::Int64, kernel_stddev::Float64, suppression_stddev::Float64)
    """
    Transforms an ambient sample (~N(0, I)) into the parameter space (~N(0, Sigma)).
    """
...
function main()
    ...
    model_dict = Dict( # We don't save the functions, so we can load this later
        ...
        :n_rate_parameters => model.n_rate_parameters
    )
